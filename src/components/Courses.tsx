        ...
        <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-2">
          {courses.map((c) => {
            ...
            return (
              <div
                key={c.title}
                className={`group relative overflow-hidden rounded-3xl border bg-white p-8 shadow-soft transition-all duration-300 hover:-translate-y-1.5 hover:shadow-glow ${
                  isBrand ? "border-brand-200 hover:border-brand-300" : "border-bio-200 hover:border-bio-300"
                }`}
              >
                {/* Decorative blob */}
                <div
                  className={`pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full blur-3xl transition-opacity duration-300 group-hover:opacity-100 ${
                    isBrand ? "bg-brand-100 opacity-60" : "bg-bio-100 opacity-60"
                  }`}
                />
                ...
                <div className="relative flex items-start justify-between">
                  ...
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-bold ${
                      isBrand ? "bg-brand-50 text-brand-700" : "bg-bio-50 text-bio-700"
                    }`}
                  >
                    {c.tag}
                  </span>
                ...
                <a
                  ...
                  className={`relative mt-7 inline-flex items-center gap-1.5 rounded-xl px-5 py-3 text-sm font-bold text-white transition-all hover:shadow-lg active:scale-95 ${
                    isBrand ? "bg-brand-600 hover:bg-brand-700 shadow-glow" : "bg-bio-600 hover:bg-bio-700 shadow-glowGreen"
                  }`}
                >
                  Yozilish
